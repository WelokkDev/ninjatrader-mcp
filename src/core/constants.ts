...
export const SUPPORTED_TIMEFRAMES: Timeframe[] = ["1m", "5m", "15m", "30m", "1h", "2h", "4h"];

// Raw timeframes are persisted directly from NT8; they are not derived
// by aggregation. 1m and 5m are parallel raw streams with no derived TFs.
// 15m additionally drives the aggregation chain in AGGREGATION_MAP below.
export const RAW_TIMEFRAMES: ReadonlyArray<Timeframe> = ["1m", "5m", "15m"];
...
export const AGGREGATION_MAP: Record<Exclude<Timeframe, "1m" | "5m" | "15m" | "1d">, number> = {
  "30m": 2,
  "1h": 4,
  "2h": 8,
  "4h": 16,
